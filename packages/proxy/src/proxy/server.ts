...
export class MitmProxyServer {
    ...
    constructor(private opts: ProxyOptions = {}) {
        ...
        this.httpServer = http.createServer((req, res) => {
            this.handleHttpRequest(req, res, false).catch((err) =>
    ...
    private async handleConnect(
        ...
        const httpOverTls = http.createServer((req2, res2) => {
            this.handleHttpRequest(req2, res2, true).catch((err) =>
                this.handleError(err, {})
            )
        })
    ...
    private async handleHttpRequest(
        ...
        const hasReqBodyHook = this.plugins.some(
            (p) => typeof p.onRequestBody === 'function'
        )
        const hasResBodyHook = this.plugins.some(
            (p) => typeof p.onResponseBody === 'function'
        )
