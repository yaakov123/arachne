...
describe('FileStorageAdapter', () => {
    it('records requests and normalizes path', async () => {
        const adapter = new FileStorageAdapter({ outDir: tmpDir })
        const ctx: any = {
            id: 'req-1',
        ...
        const key = 'GET /users/12345/profile'
        expect(host.endpoints[key]).toBeDefined()

        const ep = host.endpoints[key]
        expect(ep.method).toBe('GET')
        expect(ep.path).toBe('/users/12345/profile')
        ...
        expect(
            int.request.query.some((q: any) => q.key === 'a' && q.value === '1')
        ).toBe(true)
        expect(
            int.request.headers.some(
                (h: any) => h.key === 'x-foo' && h.value === 'bar'
            )
        ).toBe(true)
    })
})
