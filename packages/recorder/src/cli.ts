...
program
    .command('start')
    .description(
        'Start the recorder proxy and print the in-memory inventory JSON when it exits'
    )
    .option(
        '-p, --port <port>',
        'Port to listen on',
    ...
    .option(
        ...
        getDefaultCertStoreOptions().baseDir
    )
    .option(
        '--capture-bodies',
        'Capture small example request/response bodies (up to max bytes)',
        true
    )
    .option(
        '--normalize-paths',
        'Normalize likely ID segments in paths to {id}',
        true
    )
    .option(
        '--max-bytes <n>',
        'Max bytes to capture per body sample',
        (v) => parseInt(v, 10),
        1024 * 1024
    )
    ...
    .option(
        '--out <dir>',
        'Output directory for file storage (defaults to ~/.arachne/recorder)',
        os.homedir() + '/.arachne/recorder'
    )
    .action(async (opts) => {
        ...
        console.log(
            'Use this proxy in your system/browser to record traffic. Press Ctrl+C to stop and output the captured inventory JSON...'
        )
