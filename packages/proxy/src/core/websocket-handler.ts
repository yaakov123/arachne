...
import { sanitizeHeaders, shouldIgnoreHost } from './utils/headers'
import {
    createCorrelationId,
    extendCorrelationId,
    parseCorrelationId,
...
export class WebSocketHandler {
    ...
    async handleUpgrade(
        ...
        const parentCorrelation = options.connectId
            ? parseCorrelationId(options.connectId)
            : undefined
        ...
        try {
            ...
            if (!useProxyHeaders) {
                logger.debug(
                    'Creating direct WebSocket tunnel for ignored host',
                    {
                        requestId: upgradeId,
                        connectId,
                        hostname,
    ...
    private async createWebSocketTunnel(
        options: WebSocketTunnelOptions
    ): Promise<WebSocketUpgradeResult> {
        const {
            req,
            clientSocket,
        ...
        const upstreamHeaders = useProxyHeaders
        ...
        return new Promise((resolve) => {
            upstreamReq.on(
                'upgrade',
                (upstreamRes, upstreamSocket, upstreamHead) => {
                    logger.debug(
                        `${tunnelType} WebSocket tunnel upgrade successful`,
                        {
                            requestId: upgradeId,
                            connectId,
                            hostname,
            ...
            if (head && head.length > 0) {
                logger.debug(
                    `Forwarding initial client WebSocket data for ${tunnelType} tunnel`,
                    {
                        requestId: upgradeId,
                        connectId,
                        hostname,
    ...
    private forwardUpgradeResponse(
        ...
        if (upstreamHead && upstreamHead.length > 0) {
            logger.debug(
                `Forwarding initial upstream WebSocket data for ${logContext.tunnelType} tunnel`,
                {
                    ...logContext,
                    component: 'websocket-handler',
    ...
    private setupTunnelPiping(
        ...
        logger.debug(
            `Starting bidirectional WebSocket data piping for ${logContext.tunnelType} tunnel`,
            {
                ...logContext,
                component: 'websocket-handler',
            }
        ...
        Promise.allSettled([clientToUpstream, upstreamToClient]).then(() => {
            logger.debug(
                `${logContext.tunnelType} WebSocket tunnel pipelines completed`,
                {
                    ...logContext,
                    component: 'websocket-handler',
                }
        ...
        upstreamSocket.on('error', (err) => {
            logger.error(
                `Upstream WebSocket socket error in ${logContext.tunnelType} tunnel`,
                err,
                {
                    ...logContext,
                    component: 'websocket-handler',
    ...
    private handleTunnelError(
        ...
        try {
            if (!clientSocket.destroyed && clientSocket.writable) {
                logger.debug(
                    `Sending 502 Bad Gateway for ${logContext.tunnelType} WebSocket upgrade error`,
                    {
                        ...logContext,
                        component: 'websocket-handler',
                    }
            ...
            logger.error(
                `Failed to send 502 response for ${logContext.tunnelType} WebSocket upgrade error`,
                writeError,
                {
                    ...logContext,
                    component: 'websocket-handler',
