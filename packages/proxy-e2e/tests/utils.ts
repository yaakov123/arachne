...
export async function startUpstream(): Promise<Upstream> {
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/compressed/gzip') {
      ...
      zlib.gzip(raw, (e, out) => {
        if (e) { res.statusCode = 500; res.end('compress error'); return }
        res.statusCode = 200
        res.setHeader('Content-Type', 'application/octet-stream')
    ...
    if (url.pathname === '/echo-query') {
      res.statusCode = 200
      res.setHeader('Content-Type', 'application/json')
      res.end(JSON.stringify({ pathname: url.pathname, search: url.search }))
      return
    }
...
export async function requestViaProxy(proxyHost: string, proxyPort: number, target: string, opts: RequestOptions = {}) {
  const url = new URL(target)
  const method = opts.method || 'GET'
  const originForm = !!opts.originForm
  const headersLower: Record<string, string> = {}
  const headers: Record<string, string> = {}
  const addHeader = (k: string, v: string) => { headers[k] = v; headersLower[k.toLowerCase()] = v }
  ...
  if (body && !opts.streamBody && !('content-length' in headersLower)) {
    addHeader('Content-Length', String(Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body)))
  }
