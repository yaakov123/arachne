...
import type { DisplayHeader } from '@arachne/api-types'
...
const cookieItems = computed<KeyValueItem[]>(() => {
    const cookieHeaders = props.headers.filter(header => 
        header.name.toLowerCase() === 'cookie' || header.name.toLowerCase() === 'set-cookie'
    )
    ...
    for (const header of cookieHeaders) {
        if (header.name.toLowerCase() === 'cookie') {
            ...
            const cookiePairs = header.value.split(';').map(pair => pair.trim())
            ...
            if (parts.length > 0) {
                ...
                if (name && valueParts.length > 0) {
                    const attributes = parts.slice(1).filter(attr => attr.length > 0)
                    const value = valueParts.join('=').trim()
                    const displayValue = attributes.length > 0 
